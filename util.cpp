...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IZE
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IZE
    // Mac: ~/Library/Application Support/IZE
    // Unix: ~/.IZE
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IZE";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "IzeC.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "IzeCd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong IzeC will not work properly.");
