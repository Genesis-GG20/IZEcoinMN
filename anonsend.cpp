    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat IzeC and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 IZE per transaction on average.
            */
            if(r <= 10)
            {
...
bool CAnonsendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeIzeCAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
void CAnonsendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100 IZE +1 ( bit on if present )
    // bit 1 - 10 IZE +1
    // bit 2 - 1 IZE +1
    // bit 3 - .1 IZE +1
    // bit 3 - non-denom
