...
const string strMessageMagic = "IzeC Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectIzeCFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectIzeCFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : IzeC fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectIzeCFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectIzeCFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : IzeC fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectIzeCFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectIzeCFee, ignoreFees);
}
...
unsigned int GetNextTargetRequired_legacy(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    LogPrintf("IzeC retargetted using: Terminal-Velocity difficulty curve \n");
    return Terminal_Velocity_RateX(pindexLast, fProofOfStake);
...
static unsigned int GetNextTargetRequired_new(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes IzeCCoin fix for wrong retargeting difficulty by Mammix2
