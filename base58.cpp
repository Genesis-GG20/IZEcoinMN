...
namespace {
    class CIzeCAddressVisitor : public boost::static_visitor<bool> {
    private:
        CIzeCAddress *addr;
    public:
        CIzeCAddressVisitor(CIzeCAddress *addrIn) : addr(addrIn) { }
...
bool CIzeCAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CIzeCAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CIzeCAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CIzeCAddressVisitor(this), dest);
}

bool CIzeCAddress::IsValid() const {
...
CTxDestination CIzeCAddress::Get() const {
    if (!IsValid())
...
bool CIzeCAddress::GetKeyID(CKeyID &keyID) const {
...
bool CIzeCAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CIzeCSecret::SetKey(const CKey& vchSecret) {
...
CKey CIzeCSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CIzeCSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CIzeCSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CIzeCSecret::SetString(const std::string& strSecret) {
