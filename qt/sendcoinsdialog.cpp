    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a IzeC address (e.g. MLaZBUkdM15PjB7Zmbmwfwi31L3KeExgFG)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CIzeCAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("FastTx doesn't support sending values that high yet. Transactions are currently limited to 500 IZE.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    ...
    case WalletModel::NarrationTooLong:
        ...
        break;
    case WalletModel::IzeCFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an IzeCly high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CIzeCAddress addr = CIzeCAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid IzeC address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
