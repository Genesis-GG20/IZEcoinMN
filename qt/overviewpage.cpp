...
void OverviewPage::updateAnonsendProgress()
{
    ...
    QString strAnonymizeIzeCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIzeCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeIzeCAmount = strAnonymizeIzeCAmount.remove(strAnonymizeIzeCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIzeCAmount + " / " + tr("%n Rounds", "", nAnonsendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeIzeCAmount*COIN) nMaxToAnonymize = nAnonymizeIzeCAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeIzeCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeIzeCAmount));
        strAnonymizeIzeCAmount = strAnonymizeIzeCAmount.remove(strAnonymizeIzeCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIzeCAmount + " / " + tr("%n Rounds", "", nAnonsendRounds);
    } else {
