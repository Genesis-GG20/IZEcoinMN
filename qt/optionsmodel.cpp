...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIzeCAmount"))
        settings.setValue("nAnonymizeIzeCAmount", 1000);
    nAnonymizeIzeCAmount = settings.value("nAnonymizeIzeCAmount").toLongLong();
    if (settings.contains("nAnonsendRounds"))
        SoftSetArg("-anonsendrounds", settings.value("nAnonsendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIzeCAmount"))
        SoftSetArg("-anonymizeIzeCamount", settings.value("nAnonymizeIzeCAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIzeCAmount:
            return QVariant(nAnonymizeIzeCAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case AnonsendRounds:
            ...
            break;
        case AnonymizeIzeCAmount:
            nAnonymizeIzeCAmount = value.toInt();
            settings.setValue("nAnonymizeIzeCAmount", nAnonymizeIzeCAmount);
            emit AnonymizeIzeCAmountChanged(nAnonymizeIzeCAmount);
            break;
        default:
            break;
        }
